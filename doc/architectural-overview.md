...
The Vuex store includes standardized helpers for request lifecycle management:

- **Request state management** (`src/store/common.js`): Provides `RequestState` constants, `createRequestState()`, and `createRequestMutations()` for consistent loading/error handling across store modules
- **API action wrappers** (`src/store/apiHelper.js`): `withApiCall()` and `createLoadingAction()` reduce boilerplate by automatically handling request/success/failure mutations and error notifications
- **Purpose**: Standardize patterns across all store modules, reduce duplication, ensure consistent error handling
- **Details**: See `doc/data-layer.md` for usage patterns and examples
