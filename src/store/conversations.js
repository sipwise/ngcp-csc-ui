...
export default {
    ...
    actions: {
        ...
        toggleBlockOutgoing(context, number) {
			...
			let isBlocked = context.getters.callerIsBlockedOutgoing(number);
            if ((isBlocked && isWhitelist) || (!isBlocked && !isWhitelist)) {
        ...
        toggleBlockBoth(context, number) {
			// TODO: Mutations for request state handling
            let id = context.getters.getSubscriberId;
            let inAction = context.getters.actionToToggleIncomingNumber(number);
            let outAction = context.getters.actionToToggleOutgoingNumber(number);
            toggleNumberInBothLists({
                id: id,
                number: number,
				...
				// TODO: Commit failed mutation
                console.log('error toggling numbers', err.message);
            });
        }
    }
};
