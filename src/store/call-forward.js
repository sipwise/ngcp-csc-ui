    ...
    actions: {
        ...
        changePositionOfDestination(context, options) {
            ...
            let adjacentIndex = options.direction === 'up' ?
                options.index - 1 :
                options.index + 1;
            // TODO: 1. If we have Inter-DestinationSet movement of Destinations,
            // we need to update priority and both DestinationSets
            if (options.direction === 'up' && options.prevId) {
                // TODO: a. Create an API method that first gets destinationset by
                // id for prevId, gives moved dest same prio as last destination
                // in prevId (after sort) then appends moved dest, then updates
                // destinationset, then gets destinationset for moved dest,
                // removes the moved dest, and finally updates destinationset.
            } else if (options.direction === 'down' && options.nextId) {
                // TODO: b. Create an API method that first gets destinationset by
                // id for nextId, gives moved dest same prio as first
                // destination in nextId (after sort) (or just 1 prio) then
                // appends moved dest, then updates destinationset, then gets
                // destinationset for moved dest, removes the moved dest, and
                // finally updates destinationset
            } else {
