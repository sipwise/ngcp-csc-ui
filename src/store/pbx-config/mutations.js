...
export default {
    ...
    filesForSoundSetSucceeded(state, options) {
		...
		for (let i = 0; i <= state.soundSetsOrdered.length; i++) {
			// TODO: Fix, something wrong in the control below
			//if (state.soundSetsOrdered[i].id === id) {
				//state.soundSetsOrdered[i] = Object.assign(state.soundSetsOrdered[i], handlesByGroups);
			//}
		}
    },
