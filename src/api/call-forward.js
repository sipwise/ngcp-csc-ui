...
export function loadAlwaysDestinations(options) {
    return new Promise((resolve, reject)=>{
        ...
        Promise.resolve().then(()=>{
            ...
            addGroupNamesAndTimeset(res[0], 'cfu', cfuTimeset);
            addGroupNamesAndTimeset(res[1], 'cfna', cfnaTimeset);
            addGroupNamesAndTimeset(res[2], 'cfb', cfbTimeset);
            resolve({
                online: res[0],
                offline: res[1],
                busy: res[2]
...
export function addGroupNamesAndTimeset(group, groupName, timesetId) {
    group.forEach(destinationset => {
        destinationset.groupName = groupName;
        destinationset.timesetId = timesetId;
    });
    return group;
}
