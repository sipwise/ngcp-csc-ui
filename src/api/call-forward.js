// TODO: Create tests, and push draft review with API View helpers. Then remove
// API View helpers, clean up, and push final review patchset
    ...
    return new Promise((resolve, reject) => {
        Vue.http.get('/api/cfsourcesets/?subscriber_id=' + id, { params: params })
            .then(result => {
                ...
                resolve(jsonBody._embedded['ngcp:cfsourcesets']);
            }).catch(err => {
                reject(err);
            });
    });
...
export function getTimesets(id, rows) {
    let params = { page: 1, rows: rows };
    return new Promise((resolve, reject) => {
        Vue.http.get('/api/cftimesets/?subscriber_id=' + id, { params: params })
            .then(result => {
                let jsonBody = getJsonBody(result.body);
                resolve(jsonBody._embedded['ngcp:cftimesets']);
            }).catch(err => {
                reject(err);
            });
    });
...
export function getDestinationsets(id, rows) {
    ...
    return new Promise((resolve, reject) => {
        Vue.http.get('/api/cfdestinationsets/?subscriber_id=' + id, { params: params })
            .then(result => {
                let jsonBody = getJsonBody(result.body);
                resolve(jsonBody._embedded['ngcp:cfdestinationsets']);
            }).catch(err => {
                reject(err);
            });
    });
