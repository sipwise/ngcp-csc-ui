...
export function getVoiceboxGreetingByType(options) {
    // TODO: Test refacored getVoiceboxGreetingByType()
    // TODO: Make test for this and store mutations (get, delete, etc)
    return new Promise((resolve, reject) => {
        getList({
            path: 'api/voicemailgreetings/',
            root: '_embedded.ngcp:voicemailgreetings',
            params: { subscriber_id: options.id }
        }).then((result)=>{
            resolve(result);
        }).catch((err)=>{
            reject(err);
    ...
    return new Promise((resolve, reject) => {
        ...
        Promise.resolve().then(() => {
            ...
            if (_.some(greetings.items, { dir: options.data.dir })) {
                deleteVoiceboxGreetingById(greetings[0].id);
            }
