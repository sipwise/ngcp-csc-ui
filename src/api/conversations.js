...
export function playVoiceMail(id) {
    return new Promise((resolve, reject)=>{
        Vue.http.get('api/voicemailrecordings/' + id + '?format=mp3', { responseType: 'blob' })
            .then((res) => {
                let url;
                let binaryData = [];
                binaryData.push(res.blob());
                url = window.URL.createObjectURL(new Blob(binaryData, {type: "audio/mpeg"}));
                resolve(url);
            }).catch((err)=>{
                reject(err);
            });
