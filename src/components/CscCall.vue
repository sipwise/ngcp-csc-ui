    ...
    <div class="csc-call">
        ...
        <q-card flat color="secondary">
            <q-card-title>
                ...
                <span v-if="isPreparing" class="text">Start new call</span>
                <span v-else-if="isInitiating" class="text">Initiating ...</span>
                <span v-else-if="isRinging" class="text">Ringing ...</span>
                <span v-else-if="isEnded" class="text">Call ended</span>
                <span v-else class="text">Call</span>

                <q-btn round small slot="right" class="no-shadow" @click="close()" icon="clear"/>
            </q-card-title>
            <q-card-main>
                <div v-if="isTrying" class="csc-spinner">
                    <q-spinner-rings color="primary" :size="60" />
                </div>
                <div v-if="isTrying" class="phone-number">
                    {{ getNumber }}
                </div>

                <csc-media v-if="isCalling && localMediaStream != null" :stream="localMediaStream" />

                <q-field v-if="isPreparing" helper="Input a phone number" :error="validationEnabled && phoneNumberError"
                         error-label="Input a valid phone number" :count="64" dark>
                    <q-input float-label="Number" v-model="formattedPhoneNumber"
                             dark clearable max="64" @blur="phoneNumberBlur()"
