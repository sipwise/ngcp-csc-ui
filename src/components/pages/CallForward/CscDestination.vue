    ...
    export default {
        ...
        methods: {
            ...
            addDestination(destinationType) {
                let clonedDestinations = _.cloneDeep(this.destinations);
                let addDestination = 'TEST';
                let store = this.$store;
                let self = this;
                clonedDestinations.push({
                    announcement_id: null,
                    destination: '44332211',
                    priority: 1,
                    timeout: 300
                });
                console.log(clonedDestinations);
                store.dispatch('callForward/deleteDestinationFromDestinationset', {
                    // TODO: This might be an issue -> destinationsetId. Investigate
                    id: self.destinationsetId,
                    data: clonedDestinations }).then((result) => {
                        store.dispatch('callForward/loadAlwaysEverybodyDestinations');
                        showToast(self.$t('pages.callForward.addSuccessMessage', {
                            destination: addDestination
                        }));
                    }).catch((err) => {
                        showToast(self.$t('pages.callForward.addErrorMessage'));
                    });
            }
        }
    }
