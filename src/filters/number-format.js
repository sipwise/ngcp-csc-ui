...
export default function(number) {
    try {
        ...
        return normalizeDestination(number);
    } catch(err1) {
        return normalizeDestination(number);
    }
}
...
export function normalizeDestination(destination) {
    ...
    } else if (destination.auth && !isNaN(destination.auth.split(':')[0])) {
        normalizedDestination = normalizeNumber(destination.auth.split(':')[0]);
    } else if (destination.auth && destination.auth.split(':')[0] == 'custom-hours') {
